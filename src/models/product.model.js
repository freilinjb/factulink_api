 ...
 */
exports.getSubCategory = async (data, callback) => {
  try {
    let condicion = null;
    let search = '';

    condicion = data.idSubCategoria ? `WHERE v.idSubCategoria = ${data.idSubCategoria}`   : "";
    if(data.idSubCategoria == null && data.limit && data.offset >= 0) {
      if(data.search) {
          data.search = data.search.trim();
          search = `  WHERE CONCAT(v.subCategoria, v.categoria, COALESCE(v.creado_por,''),v.creado_en, v.estado) LIKE '%${data.search}%'`;
      }
      condicion += ` LIMIT ${data.limit}  OFFSET ${data.offset} `;
    }
    ...
    let total_rows = 0;
    connection.query(
      `SELECT COUNT(c.idSubCategoria) AS cantidad FROM subcategoria_v c ${search}`,
      [],
      async (error, results, fields) => {
       // total_page = results[0].cantidad;
       if(results.length > 0) {
        ...
        connection.query(
          `SELECT 
            ...
            v.creado_en,
            v.estado
          FROM subcategoria_v v
              ${search}  ${condicion}`,
          [],
          (error, results, fields) => {
            console.log('idSubCategoria: ', Math.ceil(total_rows / data.limit));
